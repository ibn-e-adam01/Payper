...
const SellerPageNavbar = ({MenuIsOpen, setMenuIsOpen}) => {
  ...
  return (
    <>
    <header className={`h-16 pt-1 w-full  z-20 flex items-center justify-between  xl:px-3 gap-5 px-4 lg:px-2.5 md:px-17 bg-zinc-950 text-white border-zinc-700 fixed md:border-b`}>
      ...
      <div>
        <Link className={`text-4xl text-[#FFC247] lg:pl-0  pl-8 font-extrabold tracking-[0.03rem] cursor-pointer`} to='/'>Attire</Link>
        </div>
        ...
        <Link to={AccountProfession? '/profile' : '/setup-your-profile'} className={`h-auto flex items-center justify-center text-[#FCC247] text-2xl `}><FontAwesomeIcon icon={faHeart} /></Link>
    ...
    <button onClick={logout} className='h-auto py-0.5 font-semibold hover:bg-[#f1a918] cursor-pointer tracking-tight w-auto px-3.5 rounded-md text-lg bg-[#e09a0c] text-black'>LOGOUT</button>
